...
export default function Sidebar({ providers, coords, selectedProvider }) {
  const totalServices = providers.length;
  const totalProviders = new Set(providers.map((p) => p.name)).size;
  const available = providers.filter((p) => p.available).length;
  const services = [...new Set(providers.map((p) => p.skills[0]))];
      ...
      <div className="grid grid-cols-1 gap-3 flex-shrink-0">
        {stats.map((s) => (
          <div key={s.label} className="bg-blue-50/70 backdrop-blur-sm rounded-2xl px-5 py-4 border border-blue-100/80 shadow-sm flex items-center gap-4">
            ...
            <div>
              <p className="text-2xl font-extrabold text-[var(--color-text)] leading-none">{s.value}</p>
